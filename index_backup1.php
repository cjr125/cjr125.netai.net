  ...
  <div class="container">
    ...
    <ul id="#spotId">
      <?php
        ...
        $currentPageTracks = 0;
        $totalTracks;
        $trackListResult = DBQuery("SELECT m.*, AVG(r.rating) as rating FROM cjrmusic m JOIN ratings r ON r.id_post = m.title GROUP BY m.title ORDER BY rating DESC");
        while ($trackResult = $trackListResult->fetch_object()) {
	  $file = $trackResult->location;
          $name = $trackResult->title;
	  if ($totalTracks <= $pageSize * $page && $totalTracks > ($pageSize * ($page - 1))) {
	    $query = DBQuery("SELECT * FROM ratings WHERE id_post = '".$name."'");
	    $sum_rates = array();
	    while($data = $query->fetch_object()){
	      $sum_rates[] = $data->rating;
	    }
	    if($query->num_rows){
	      $rate_times = $query->num_rows;
	  ...
	  echo '<li><span class="trackName">'.$name.'</span><audio controls="controls"><source src="audio/'.$file.'" type="audio/mpeg" /></audio>
